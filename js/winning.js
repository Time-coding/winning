...
function Winning(options) {
	return new Winning.prototype.init(options);
}
Winning.prototype = {
	constructor: Winning,
	init: function (options) {
		this.options = options || {};
		this.options.angles = this.options.angles || 180;
		this.options.clickNum = this.options.clickNum || 1;
		this.options.rotNum = this.options.rotNum || 0;
		this.options.notice = this.options.notice || null;
		this.options.num=this.options.num||[];
		this.options.color = this.options.color || [];
		this.options.info = this.options.info || [];
		this.options.info1 = this.options.info1 || [];
		this.render();
	}
}
Winning.fn = Winning.prototype.init.prototype;
Winning.fn.render = function () {
	var _this = this;
	this.canvasRun();
	$('#tupBtn').bind('click', function () {
		if (_this.options.clickNum >= 1) {
			//可抽奖次数减一
			_this.options.clickNum = _this.options.clickNum - 1;
			//转盘旋转
			_this.runCup();
			//转盘旋转过程“开始抽奖”按钮无法点击
			$('#tupBtn').attr("disabled", true);
			//旋转次数加一
			_this.options.rotNum = _this.options.rotNum + 1;
			//“开始抽奖”按钮无法点击恢复点击
			setTimeout(function () {
				alert(_this.options.notice);
...
Winning.fn.runCup = function () {
	var angles=this.probability();
...
Winning.fn.probability = function () {
	//获取随机数
	// var num = parseInt(Math.random() * (7 - 0 + 0) + 0);
	var num=this.options.num;
	var index=Math.floor((Math.random()*num.length));
	var num=num[index];
	var angles=this.options.angles;
	var rotNum=this.options.rotNum;
	var notice;
	var info=this.options.info;
	var info1=this.options.info1;
	//概率
	if (num == 0) {
...
Winning.fn.canvasRun = function () {
	...
	var length=this.options.info1.length;
	createCircle();
	createCirText();
	initPoint();
	...
	function createCircle() {
		...
		for (var i = 0; i < length; i++) {
			startAngle = Math.PI * (i / (length/2) - 1 / length);
			endAngle = startAngle + Math.PI * (1 / (length/2));
			ctx.save();
			ctx.beginPath();
	...
	function createCirText() {
		...
		for (var i = 0; i < length; i++) {
			...
			// ctx.fillText(info[i], -50, -125, 60);
			ctx.font = " 14px Microsoft YaHei";
			ctx.fillText(info1[i], -30, -95, 60);
			// ctx.fillText(info1[i], -50, -105, 60);
			ctx.closePath();
			ctx.restore();
		}
	}
